...
void RmArm::Arm_Movej_CANFD_Callback(rm_ros_interfaces::msg::Jointpos::SharedPtr msg)
{
    ...
    this->Movej_CANFD_Result->publish(movej_CANFD_data);
...
void Udp_Robot_Status_Callback(rm_realtime_arm_joint_state_t data)
{
    ...
    // Error contents can change while err_len remains unchanged.  Refresh the
    // complete payload every cycle so a cleared/replaced error is not cached.
...
void UdpPublisherNode::udp_timer_callback() 
{
    if(connect_state == 0)
    {
        ...
        // Do not key updates only on the array length: the value can change
        // while the number of active errors stays the same.
...
/***********************************************************************运动配置**********************************************************************/
    ...
    Movej_CANFD_Cmd = this->create_subscription<rm_ros_interfaces::msg::Jointpos>("rm_driver/movej_canfd_cmd",rclcpp::ParametersQoS(),
        ...
        sub_opt4);
    Movej_CANFD_Result = this->create_publisher<std_msgs::msg::UInt32>(
